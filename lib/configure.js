...
module.exports = function(overrides) {
  ...
  if (_.isUndefined(overrides.prefixThemes)) { delete overrides.prefixThemes; }
  if (_.isUndefined(overrides.colors)) { delete overrides.colors; }
  if (_.isUndefined(overrides.logLevels)) { delete overrides.logLevels; }
  ...
  var options = _.extend(_.clone(rconf), overrides, {
    ...
    colors: (function (){
      ...
      else if (_.isObject(overrides.colors)) { return _.extend({}, rconf.colors||{}, overrides.colors); }
      else { return _.clone(overrides.colors); }
    })(),

    logLevels: (function (){
      if (_.isUndefined(overrides.logLevels)) { return _.clone(rconf.logLevels); }
      else if (_.isObject(overrides.logLevels)) { return _.extend({}, rconf.logLevels||{}, overrides.logLevels); }
      else { return _.clone(overrides.logLevels); }
    })()

  });
  if (_.isUndefined(options.prefixThemes)) { delete options.prefixThemes; }
  if (_.isUndefined(options.colors)) { delete options.colors; }
  if (_.isUndefined(options.logLevels)) { delete options.logLevels; }
  ...
  if (_.isUndefined(finalOptions.prefixThemes)) { delete finalOptions.prefixThemes; }
  if (_.isUndefined(finalOptions.colors)) { delete finalOptions.colors; }
  if (_.isUndefined(finalOptions.logLevels)) { delete finalOptions.logLevels; }
